...
SELECT * FROM SalesLT.Customer; -- this is also a comment
...
-- select with where clause (filters the rows)
...
-- with group by
-- returns first names and how many duplicates they have (for first names starting with 'A')
...
GROUP BY FirstName
HAVING COUNT(FirstName) > 1
ORDER BY COUNT(FirstName) DESC; -- default is ASC